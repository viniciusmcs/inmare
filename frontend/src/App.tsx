...
function PropertyDetailPage() {
  ...
  const primaryImage = property.media.find((media) => media.kind === "image" && media.is_primary) || property.media.find((media) => media.kind === "image");
  const description = property.public_description || `${property.property_type} em ${property.neighborhood}, ${property.city}.`;
  const propertyJsonLd = {
    ...
    floorSize: property.private_area ? { "@type": "QuantitativeValue", value: property.private_area, unitCode: "MTK" } : undefined,
...
function AboutPage() {
  ...
  return <Shell><Seo title="Sobre a In Mare" description="Conheça a In Mare Negócios Imobiliários e nosso atendimento consultivo para compra, venda e investimento em imóveis." path="/imobiliaria" structuredData={organizationSchema()} /><PageHero eyebrow="A IMOBILIÁRIA" title="Conectando pessoas a imóveis únicos" text="Atendimento humano, consultivo e altamente qualificado." />
...
function DevelopmentsPage() { return <Shell><Seo title="Empreendimentos imobiliários" description="Empreendimentos selecionados pela In Mare para quem busca localização, qualidade e exclusividade." path="/empreendimentos" structuredData={organizationSchema()} /><PageHero eyebrow="EMPREENDIMENTOS" title="Projetos para viver o extraordinário" text="Uma seleção preparada para quem busca localização, qualidade e exclusividade." /><Empty title="Empreendimentos em preparação" /></Shell>; }
